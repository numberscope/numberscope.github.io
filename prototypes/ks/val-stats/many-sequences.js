...
function manySeq(p) {
	...
	this.createVariety = function(bd) {
		...
		mySeqs.push([this.transformNadic(mySeqs[0][0],2),'2-adic val of Z']);
		mySeqs.push([this.createRandom(bd,2),'Random Binary']); // 3
		mySeqs.push([this.createRandom(bd,4),'Random modulo 4']); // 4
		mySeqs.push([this.createRandom(bd,30),'Random modulo 30']); // 5
		mySeqs.push([this.createRandom(bd,1009),'Random modulo prime 1009']); // 6
		...
		mySeqs.push([this.createDivisors(bd),'Number of divisors']);
		mySeqs.push([this.createPrimes(bd),'Primes']);
		//mySeqs.push([this.createEphi(bd),'Euler Phi']);
		//mySeqs.push([this.createSigma(bd),'Sigma 2']);
		//mySeqs.push([this.createRamTau(bd),'Ramanujan Tau']);
		//mySeqs.push([this.createPartitions(bd),'Partitions']);
		//mySeqs.push([this.createSumDiv(bd),'Sum of divisors']);
		mySeqs.push([this.createHofstadterQ(bd),'Hofstadter Q']);
		return mySeqs
	}
	...
	this.createPartitions = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(partSeq[i]);
		}
		return myinseq;
	}
	

	this.createRamTau = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(ramtauSeq[i]);
		}
		return myinseq;
	}
	
	this.createSumDiv = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(sumdivSeq[i]);
		}
		return myinseq;
	}
	...
	this.createSigma = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(sigmaSeq[i]);
		}
		return myinseq;
	}
	...
	this.createEphi = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(ephiSeq[i]);
		}
		return myinseq;
	}
	...
	this.createPrimes = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(primesSeq[i]);
		}
		return myinseq;
	}
	...
	this.createDivisors = function(bd) {
		var myinseq = [];
		for( var i = 1; i < bd; i++ ){
			myinseq.push(divisorsSeq[i]);
		}
		return myinseq;
	}
