var simTelescope = function( p ) {
	...
	var fromColor = p.color(218, 165, 32); // yellow
	...
	p.freshDraw = function() {
		...
		for (let j = 2; j < p.floor(p.height/zoomFactor)-1; j++) { // y coordinate (row)
			for (let i = 2; i < p.floor(p.width/zoomFactor)-1; i++) { // x coordinate (column entries in row)
				...
				//c = p.lerpColor(fromColor, toColor, modulo(mySeq[p.floor(indexFun)],modulus)/modulus);
				...
				if( indexFun > 0 && indexFun < mySeq.length && i-2 < mySeq.length && baseVal != Infinity && compareVal != Infinity ){
					...
					if( mode == 1 ){ // gcd (divisibility) difference mode
						...
						//console.log(baseVal,compareVal,mygcd,mymin,mymax);
						//diff = p.floor( math.log(mymin) - math.log(mygcd) );
						//diff = mymin/mygcd; // - one;
					}
					...
					myfade = bigclass.min(diff,nfade);
					if( typeof(myfade) == "bigint" && myfade > BigInt(255)*nfade ){ myfade = 255*fade }else{ myfade = Number(myfade) };
					if( diff < 100 || diff < 100n ){
						//console.log(baseVal, compareVal, diff, myfade, fade);
					}
					c = p.lerpColor(fromColor,toColor,myfade/fade); // some problem defaults to paleMist now and again?
					//coolColor = p.lerpColor(c,p.color('#5fc066'),0.5);
					mycircsize = zoomFactor/2.1;
					if( mode == 1 ){ 
						if( mygcd == 0 ){ lerp = 0 }else{
							minlen = mymin.toString(2).length;
							maxlen = mymax.toString(2).length;
							gcdlen = mygcd.toString(2).length;
							ratio = (gcdlen/minlen)**(3/fade);
							lerp = ratio;
							//console.log( baseVal, compareVal, mygcd, mymin, mymax, ratio, ratioStr, lerp);
							//	ratio = mygcd/mymax;
							//	lerp = p.min(255,fade*ratio);
						}
						if( bigclass.abs(baseVal) > bigclass.abs(compareVal) ){ 
							c = p.lerpColor(toColor,coolColor,lerp);
						} else {
							c = p.lerpColor(toColor,fromColor,lerp);
						}
						//c = p.lerpColor(fromColor,coolColor,mygcd/mymax);
						//c = p.lerpColor(c,coolColor,mygcd/mymax); 
						if( mymin == 0 ){
							c = p.color(100,100,100);
							mycircsize = mycircsize*0.4;
						}
					}
					if( mode == 0 && diff == 0 ){ c = fromColor; }
					p.fill(c);
					p.strokeWeight(0);
					...
					//c = p.lerpColor(fromColor, toColor, p.abs(baseVal-compareVal)/3);
				}
			}
		}
	}
	...
	p.bigMenu = function() {
		...
		if( mode == 0 ){
			...
			p.translate(0,lineskip);
			p.text("The brightness of a star indicates the absolute",0,0);
			p.translate(0,lineskip);
			p.text("value of the distance between the compared terms.",0,0);
			p.translate(0,lineskip);
		}
		if( mode == 1 ){
			p.text("You are using divisibility detection mode.",0,0);
			p.translate(0,lineskip);
			p.text("The brightness of the star indicates min/gcd - 1.",0,0);
			p.translate(0,lineskip);
			p.text("The colour of the star indicates which term is bigger (hue with gcd/max).",0,0);
			p.translate(0,lineskip);
		}
		if( mode == 2 ){
		...
		p.translate(0,lineskip);
		p.text("0:  toggle background mist that indicates indices repeated from earlier", 0,0);
		p.translate(0,lineskip);
		p.translate(0,lineskip);
		...
		p.translate(0,lineskip);
		p.text("translation t (right/left arrow; s/d or S/D larger step; t for one):   "+ p.nf(translat,1,2),0,0);
		p.pop();
	}
