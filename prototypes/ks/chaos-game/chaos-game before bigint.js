	...
	p.draw = function() {
		...
		if( pixelCount + pixelsPerFrame < mySeq.length ){
			...
			for( let i = 0; i < pixelsPerFrame; i++ ){
				...
				if( p.abs(indCorSeq) < Number.MAX_SAFE_INTEGER ){

					// move that direction
					cornerNumSeq = modulo(indCorSeq,poly);
					...
					if( colorStyle == 1 ){ // colour by destination
						myColSeq = colorPalette[modulo(modulo(cornerNumSeq,poly),colorPalette.length)];
						myColSeq.setAlpha(basicAlpha);
					}
	...
	p.keyTyped = function() {
		switch( p.key ){
			...
			case '1':
				...
				p.frameRate(fr);
				break;
			case '9': // change highlighted walker
				highlightWalker = modulo(highlightWalker-1,numWalkers);
				p.preDraw();
				break;
			case '0': // change highlighted walker
				highlightWalker = modulo(highlightWalker+1,numWalkers);
				p.preDraw();
				break;
